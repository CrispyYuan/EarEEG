...
def parse_option():
    ...
    #代码在读取数据时，是把所有文件当成一个列表，然后通过列表的索引（0开始）去取数据的，而不是通过文件名里那个数字。
    parser.add_argument('--train_data_list', nargs="+", default = ['[0,1,2,3,4,5,6]'] ,  help='Folds in the dataset for training')
    parser.add_argument('--val_data_list', nargs="+", default = ['[7]'] ,  help='Folds in the dataset for validation')
    ...
    parser.add_argument('--n_epochs', type=int, default = 30 ,  help='No of training epochs')
...
def main():
    ...
    if args.training_type == 'supervised':
        ...
        # 1. 从 train_data_loader 绑定的 dataset 中提取我们刚刚保存的 weights
        # 注意：这里的 train_data_loader 是 DataLoader 对象，它的 .dataset 属性才是我们修改的类
        class_weights = torch.FloatTensor(train_data_loader.dataset.weights).to(device)
        # 2. 把权重传入交叉熵损失函数
